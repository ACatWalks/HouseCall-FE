    ...
    async function handleSubmit(e) {
        ...
        for(let i=0; i<inputs.length; i++){
            if(inputs[i].checked === true){
                startingSymptoms.push(inputs[i].value)
            } else if(startingSymptoms.includes(inputs[i].value)){
                startingSymptoms.splice(startingSymptoms.indexOf(inputs[i].value), 1)
            }
        }
        let uniqueSymptoms = [...new Set(startingSymptoms)]
        let symptoms = [...uniqueSymptoms]
        user.symptoms = symptoms
        await fetch(`http://house-calls-be.herokuapp.com/patients/${user.email}`, {
            method: 'PUT',
            headers: {
